...
const STATE_DATA_KEYS=['chits','members','auctions','payments','reminders','activityLog','notifications'];
function buildRestoredState(d){const out={};STATE_DATA_KEYS.forEach(k=>{out[k]=Array.isArray(d?.[k])?d[k]:[]});return out}
function snapshotBeforeRestore(){try{localStorage.setItem(BACKUP_PRERESTORE_KEY,JSON.stringify(backupPayload()));localStorage.setItem(BACKUP_PRERESTORE_KEY+'_time',new Date().toISOString());}catch(e){}}
async function pullFromSupabase(){if(!supa||syncBusy)return false;syncBusy=true;try{const {data,error}=await supa.from("mithraq_data").select("data,updated_at").eq("id","main").maybeSingle();if(error)throw error;if(data?.data){snapshotBeforeRestore();state=buildRestoredState(data.data);save();supaStatus="Downloaded";render();return true}supaStatus="Connected (no cloud data yet)";return false}catch(e){supaStatus="Sync error: "+(e.message||"unknown");return false}finally{syncBusy=false}}
...
function auctionRecordHtml(a,c){
  ...
  const overdue=auctionOverdue(a);
  const risk=m?memberRiskFlags(m.id,a.id):[];
  return `<div class="card auction-history">
    <div class="auction-main"><div class="winner-badge">🏆</div><div style="flex:1"><b>${esc(a.member||'Winner')}</b>${risk.length?`<span class="risk-flag" title="${esc(risk.join(' • '))}">⚠ Risk</span>`:''}<div class="muted">${esc(a.chit||c.name)} • ${esc(a.round||'Round 1')} • ${esc(a.date||'')}</div></div><div class="auction-amount"><b>${money(bid)}</b><small>Winner</small></div></div>
    <div class="auction-receipt-grid">
      ...
      <div class="payable-cell"><span>PAYABLE</span><b>${money(fin.payable)}</b></div><div><span>WINNER PAYMENT</span><b class="wp-status wp-${esc(status.toLowerCase())}">${esc(status)}${overdue?' • OVERDUE':''}</b></div>
    </div>
    ${risk.length?`<div class="risk-note">⚠ ${esc(risk.join(' • '))}</div>`:''}
    ${status!=='Paid'?`<div class="winner-payment-actions">${status!=='Partial'?`<button class="action-btn" onclick="setWinnerPaymentStatus('${String(a.id)}','Partial')">◐ Mark Partial</button>`:''}<button class="action-btn edit" onclick="setWinnerPaymentStatus('${String(a.id)}','Paid')">✓ Mark Paid</button><button class="action-btn" onclick="sendWinnerPaymentReminder('${String(a.id)}')">💬 Remind Payment</button></div>`:''}
    ${prizeBlockHTML(a)}<div class="auction-due">Due date: <b>${formatDMY(a.dueDate)}</b></div>
    ${auctionBidHistoryHTML(a)}
...
function auctionOverdue(a){if(!a||(a.paymentStatus||'Pending')==='Paid'||!a.dueDate)return false;return new Date(a.dueDate)<new Date(new Date().toDateString());}
function setWinnerPaymentStatus(id,status){const a=state.auctions.find(x=>String(x.id)===String(id));if(!a)return;a.paymentStatus=status;if(status==='Paid')a.paymentPaidAt=new Date().toISOString();save();logActivity('Winner payment marked '+status,'Payment',`${a.member||'Winner'} • ${a.chit||''} • ${money(a.payable||0)}`);render();}
function winnerPaymentReminderText(a,c,m){return `MithraQ Payment Reminder\n\nDear ${m.name},\nYour ${c?.name||a.chit} chit auction (${a.round||'Round 1'}) payable amount is still pending.\n\nPayable: ${money(a.payable||0)}\nDue Date: ${formatDMY(a.dueDate)}\n\nPlease pay at the earliest. Thank you.`;}
function sendWinnerPaymentReminder(id){const a=state.auctions.find(x=>String(x.id)===String(id));if(!a)return;const c=chitById(a.chitId),m=state.members.find(x=>String(x.id)===String(a.memberId));if(!m)return uiAlert('Winner details not found.');if(!waOpenOrWarn(m.phone,winnerPaymentReminderText(a,c,m)))return;a.paymentReminders=Array.isArray(a.paymentReminders)?a.paymentReminders:[];a.paymentReminders.push(new Date().toISOString());save();logActivity('Winner payment reminder sent','Payment',`${a.member||'Winner'} • ${a.chit||''}`);render();}
function memberRiskFlags(memberId,excludeAuctionId){const reasons=[];const today=new Date(new Date().toDateString());const overdueWins=(state.auctions||[]).filter(x=>String(x.memberId)===String(memberId)&&String(x.id)!==String(excludeAuctionId)&&(x.paymentStatus||'Pending')!=='Paid'&&x.dueDate&&new Date(x.dueDate)<today);if(overdueWins.length)reasons.push(`${overdueWins.length} overdue prize payment(s) on other rounds`);const m=state.members.find(x=>String(x.id)===String(memberId));if(m){const month=new Date().toISOString().slice(0,7);const pend=(m.chitIds||[]).map(chitById).filter(Boolean).reduce((s,c)=>{const expected=memberMonthly(m,c),p=paymentFor(m.id,month,c.id),paid=Number(p?.amount||0);return s+Math.max(0,expected-paid);},0);if(pend>0)reasons.push(`${money(pend)} pending in this month's installments`);}return reasons;}
function auctionBidHistoryHTML(a){
  const list=Array.isArray(a.bidHistory)?a.bidHistory.slice().sort((x,y)=>Number(y.bid||0)-Number(x.bid||0)):[];
  if(!list.length)return `<div class="auction-bids-note">Detailed bid-by-bid history was not recorded for this round.</div>`;
  return `<details class="auction-bids-details"><summary>View all ${list.length} bid(s) this round</summary><div class="auction-bids-list">${list.map((b,i)=>`<div class="auction-bid-row${String(b.memberId)===String(a.memberId)&&Number(b.bid)===Number(a.bid)?' win':''}"><span>#${i+1}</span><div style="flex:1"><b>${esc(b.memberName||'')}</b><div class="muted">#${esc(b.memberNo||'—')}</div></div><strong>${money(b.bid)}</strong></div>`).join('')}</div></details>`;
}
...
const BACKUP_PRERESTORE_KEY='mithraq_pre_restore_v1';
...
function restoreSnapshot(){const raw=localStorage.getItem(BACKUP_SNAPSHOT_KEY);if(!raw)return uiAlert('No automatic backup snapshot is available yet.');try{const x=JSON.parse(raw),d=x.data||x,v=backupValidateData(d);if(!v.ok)return uiAlert(v.msg);uiConfirm('Restore the latest automatic backup? Current data will be replaced.',()=>{snapshotBeforeRestore();state=buildRestoredState(d);state.members.forEach(normalizeMember);_backupSave();uiAlert('Automatic backup restored successfully.');tab='home';render();});}catch(e){uiAlert('Automatic backup snapshot is corrupted.');}}
function undoLastRestore(){const raw=localStorage.getItem(BACKUP_PRERESTORE_KEY);if(!raw)return uiAlert('No pre-restore snapshot is available yet.');try{const x=JSON.parse(raw),d=x.data||x,v=backupValidateData(d);if(!v.ok)return uiAlert(v.msg);uiConfirm('Undo the last restore and bring back the data from right before it? This will replace your current data.',()=>{state=buildRestoredState(d);state.members.forEach(normalizeMember);_backupSave();uiAlert('Pre-restore data brought back successfully.');tab='home';render();});}catch(e){uiAlert('Pre-restore snapshot is corrupted.');}}
function clearAllMithraQData(){uiConfirm('Clear all MithraQ business data from this browser? A downloadable backup is recommended first.',()=>{backupDownloadJSON();setTimeout(()=>uiConfirm('Final confirmation: permanently clear members, chits, payments, auctions and reminders on this device?',()=>{state={chits:[],members:[],auctions:[],payments:[],reminders:[],activityLog:[],notifications:[]};_backupSave();localStorage.removeItem(BACKUP_SNAPSHOT_KEY);localStorage.removeItem(BACKUP_SNAPSHOT_KEY+'_time');localStorage.removeItem(BACKUP_PRERESTORE_KEY);localStorage.removeItem(BACKUP_PRERESTORE_KEY+'_time');uiAlert('All business data was cleared from this device.');tab='home';render();}),250);});}
function backupCenter(){const c=backupRecordCounts(),snapTime=localStorage.getItem(BACKUP_SNAPSHOT_KEY+'_time'),preTime=localStorage.getItem(BACKUP_PRERESTORE_KEY+'_time'),auto=backupAutoEnabled();return `<div class="card backup-center"><div class="row"><div><h3 style="margin:0">💾 Backup & Restore Center</h3><div class="muted">Protect your MithraQ data on this device.</div></div><span class="badge ${auto?'active':'pending'}">${auto?'AUTO ON':'AUTO OFF'}</span></div><div class="backup-count-grid"><div><b>${c.members}</b><span>Members</span></div><div><b>${c.chits}</b><span>Chits</span></div><div><b>${c.payments}</b><span>Payments</span></div><div><b>${c.auctions}</b><span>Auctions</span></div><div><b>${c.reminders}</b><span>Reminders</span></div></div><div class="backup-actions"><button class="btn gold" onclick="backupDownloadJSON()">⬇️ Download JSON Backup</button><button class="btn" onclick="restoreSnapshot()">↩️ Restore Auto Backup</button><button class="btn" onclick="backupSnapshotNow();uiAlert('Automatic snapshot created successfully.');render()">📸 Create Snapshot</button><button class="btn" onclick="document.getElementById('restoreFile')?.click()">📤 Restore JSON File</button></div><label class="backup-toggle"><input type="checkbox" ${auto?'checked':''} onchange="setAutoBackup(this.checked)"><span><b>Automatic local snapshot</b><small>Keep the latest backup inside this browser after data changes.</small></span></label><div class="backup-meta"><span>Latest snapshot: <b>${snapTime?new Date(snapTime).toLocaleString('en-IN'):'Not created'}</b></span><span>Storage: <b>Local Storage</b></span></div>${preTime?`<div class="backup-note">A safety copy of your data from just before your last restore is saved (${esc(new Date(preTime).toLocaleString('en-IN'))}). <button class="btn" onclick="undoLastRestore()">↩️ Undo Last Restore</button></div>`:''}<button class="btn danger-outline full" onclick="clearAllMithraQData()">🗑️ Clear All Business Data</button></div>`}
