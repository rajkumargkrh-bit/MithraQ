...
function members(){if(!state.chits.length)return `<div class="row"><div><h2 class="page-title">Members</h2><div class="subtitle">Create a chit group first.</div></div></div><div class="empty big-empty"><div class="empty-icon">♙</div><b>No chit groups available</b><p>Add a chit first. Members will be added <strong>group-wise</strong> only to the chit you select.</p><button class="btn gold" onclick="newChit()">+ Create Chit</button></div>`;return `<div class="row"><div><h2 class="page-title">Members</h2><div class="subtitle">${state.members.length} registered members • Chit wise</div></div><button class="btn gold" onclick="newMember()">+ Add</button></div><div class="accordion-list">${state.chits.map(c=>{const n=membersForChit(c.id).length,cap=Number(c.duration||0),full=cap>0&&n>=cap;return `<div class="accordion-item" id="acc-${c.id}"><div class="accordion-header" onclick="toggleChitAccordion('${String(c.id)}')"><div><div class="chit-title">${esc(c.name)}</div><div class="muted">${n}/${cap||'∞'} members${full?' • FULL':''} • Monthly ${money(c.monthly||0)}</div></div><span class="accordion-arrow">▾</span></div><div class="accordion-body"><div class="list">${groupMemberPanel(c.id)}</div></div></div>`}).join('')}</div>`;}
...
function newMember(preselect=''){if(!state.chits.length)return alert('Please create a chit group first.');openModal('Add Member — Select Chit Group',`<div class="form"><label class="field-label">Chit Group <span>*</span></label><select id="mChit">${state.chits.map(c=>{const n=membersForChit(c.id).length,cap=Number(c.duration||0),full=cap>0&&n>=cap;return `<option value="${c.id}" ${String(c.id)===String(preselect)?'selected':''} ${full?'disabled':''}>${esc(c.name)} • ${c.type==='dividend'?'Dividend':'Fixed'} • ${money(c.monthly||0)}/month • ${n}/${cap||'∞'}${full?' • FULL':''}</option>`}).join('')}</select><label class="field-label">Member details</label><input id="mName" placeholder="Member name"><input id="mPhone" type="tel" placeholder="Phone number"><input id="mNo" placeholder="Member number (optional)"><input id="mDate" type="date" value="${new Date().toISOString().slice(0,10)}"><input id="mMonthly" type="number" placeholder="Monthly amount ₹ (optional)"><button class="btn gold full" onclick="createMember()">Add to Selected Chit</button><div class="form-note">Only members added to the selected chit will appear in that group, and only those members can be used for that group's auction. A chit can only hold as many members as its duration (months) — once full, no more can be added.</div></div>`);}
function createMember(){const chitId=document.getElementById('mChit').value,name=document.getElementById('mName').value.trim();if(!name)return alert('Enter member name.');const c=chitById(chitId);if(!c)return alert('Please select a chit group.');const cap=Number(c.duration||0),existing=membersForChit(chitId).length;if(cap>0&&existing>=cap)return alert(`"${c.name}" is already full (${cap}/${cap} members). This chit's duration is ${cap} months, so it cannot hold more than ${cap} members. Increase the chit's duration first if you need to add more.`);const monthly=Number(document.getElementById('mMonthly').value||c.monthly||0);state.members.push({id:Date.now(),name,phone:document.getElementById('mPhone').value.trim(),memberNo:document.getElementById('mNo').value.trim(),joiningDate:document.getElementById('mDate').value,chitIds:[chitId],monthly});save();closeModal();tab='members';render();setTimeout(()=>{const item=document.getElementById('acc-'+chitId);if(item){item.classList.add('open');item.scrollIntoView({behavior:'smooth',block:'start'});}},0);}
...
function saveAuction(){const c=chitById(auctionGroupValue)||chitById(document.getElementById('auctionChit')?.value),mid=document.getElementById('auctionMember')?.value,bid=Number(document.getElementById('bid')?.value||0);if(!c||!mid||!bid)return alert('Select a member and enter the winning bid amount.');if(bid>Number(c.amount||0))return alert('Bid cannot be greater than the chit value.');const m=state.members.find(x=>String(x.id)===String(mid));if(!m||!membersForChit(c.id).some(x=>String(x.id)===String(mid)))return alert('Selected member does not belong to this chit group.');const dueDate=document.getElementById('dueDate')?.value||defaultDueDate();const fin=auctionFinance(c,bid);const rec={id:Date.now(),chit:c.name,chitId:c.id,member:m.name,memberId:m.id,bid,round:'Round '+auctionRoundValue,chitAmount:fin.chitAmount,dividend:fin.discount,commissionAmt:fin.commissionAmt,dividendPerMember:fin.dividendPerMember,payable:fin.payable,dueDate,date:new Date().toLocaleDateString('en-IN'),createdAt:new Date().toISOString()};state.auctions.unshift(rec);save();clearInterval(auctionTimer);auctionRunning=false;alert('Auction winner saved successfully.');render();}
