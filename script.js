...
let state=JSON.parse(localStorage.getItem(KEY)||'null')||{chits:[],members:[],auctions:[],payments:[]};
...
function auction(){if(!state.chits.length)return `<h2 class="page-title">Auction Room</h2><div class="subtitle">Select a chit group first.</div><div class="empty big-empty"><b>No chit groups available</b><p>Create a chit before starting an auction.</p><button class="btn gold" onclick="newChit()">+ Create Chit</button></div>`;const c=state.chits[0],ms=membersForChit(c.id);return `<h2 class="page-title">Auction Room</h2><div class="subtitle">Only members belonging to the selected group can bid.</div><div class="card"><div class="section no-margin"><h3>Live Auction Entry</h3><span class="badge pending">READY</span></div><div class="form"><label class="field-label">Chit Group</label><select id="auctionChit" onchange="renderAuctionMembers()">${state.chits.map(x=>`<option value="${x.id}">${esc(x.name)} • ${x.type==='dividend'?'Dividend':'Fixed'}</option>`).join('')}</select><div id="auctionMembers">${auctionMemberSelect(ms)}</div><input id="bid" type="number" placeholder="Bid / auction discount ₹"><div class="timer-card"><div><span>ROUND</span><strong id="roundNo">1</strong> <small>/ 3</small></div><div id="timer">00:30</div><button class="timer-btn" onclick="startAuctionTimer()">START</button></div><button class="btn gold full" onclick="saveAuction()">Confirm Bid / Winner</button></div></div><div class="section"><h3>Auction History</h3></div><div class="list">${state.auctions.map(a=>`<div class="card row"><div><b>${esc(a.chit)}</b><div class="muted">${esc(a.member||'')} • ${esc(a.round||'Final')} • ${a.date}</div></div><b>${money(a.bid)}</b></div>`).join('')||'<div class="empty">No auctions recorded.</div>'}</div>`;}
function auctionMemberSelect(ms){return `<select id="auctionMember">${ms.length?ms.map(m=>`<option value="${m.id}">${esc(m.name)} • #${esc(m.memberNo||'—')}</option>`).join(''):'<option value="">No members in this group</option>'}</select><div class="form-note">${ms.length} member(s) available for this chit only.</div>`;}
function renderAuctionMembers(){const c=chitById(document.getElementById('auctionChit').value);document.getElementById('auctionMembers').innerHTML=auctionMemberSelect(membersForChit(c.id));}
let timer=null,seconds=30,round=1;
function startAuctionTimer(){clearInterval(timer);seconds=30;document.getElementById('roundNo').textContent=round;document.getElementById('timer').textContent='00:30';timer=setInterval(()=>{seconds--;document.getElementById('timer').textContent='00:'+String(seconds).padStart(2,'0');if(seconds<=0){clearInterval(timer);if(round<3){round++;alert('Round '+(round-1)+' complete. Starting Round '+round+'.');document.getElementById('roundNo').textContent=round;document.getElementById('timer').textContent='00:30';startAuctionTimer();}else{alert('Final Round complete. Select the winner and confirm bid.');round=1;} }},1000);}
function saveAuction(){const c=chitById(document.getElementById('auctionChit')?.value),mid=document.getElementById('auctionMember')?.value,bid=Number(document.getElementById('bid')?.value||0);if(!c||!mid||!bid)return alert('Select a group/member and enter bid amount.');const m=state.members.find(x=>String(x.id)===String(mid));state.auctions.unshift({id:Date.now(),chit:c.name,chitId:c.id,member:m?.name||'',memberId:mid,bid,round:'Round '+round,date:new Date().toLocaleDateString('en-IN')});save();render();}
