...
function authScreen(){const a=getAuth();const memberMode=window.__memberLoginMode===true;
...
async function loginAdmin(){await __baseLoginAdminPhase12();if(authUser){currentRole='admin';currentMemberId=null;window.__memberLoginMode=false;}}
function logoutAdmin(){uiConfirm('Logout from MithraQ?',()=>{authUser=null;currentRole='admin';currentMemberId=null;window.__memberLoginMode=false;showLogin();});}
...
function render(){if(currentRole==='member'){document.getElementById('app').innerHTML=memberPortal();document.querySelectorAll('.bottom-nav,.topbar').forEach(x=>x.style.display='none');return;}document.querySelectorAll('.bottom-nav,.topbar').forEach(x=>x.style.display='');__baseRenderPhase12();}
...
function settingsPanel(){return __baseSettingsPhase12()+memberAccountPanel();}
