...
state.auctions=Array.isArray(state.auctions)?state.auctions:[]; state.payments=Array.isArray(state.payments)?state.payments:[]; state.reminders=Array.isArray(state.reminders)?state.reminders:[];
let tab="home";
...
function savePayment(memberId,chitId,month){
  ...
  if(p){
    ...
    p.member=m.name; p.memberPhone=m.phone||''; p.chit=c.name; p.chitId=c.id;
    p.status=paymentStatus(p,memberMonthly(m,c));
    p.updatedAt=new Date().toISOString();
  }else{
    p={id:Date.now(),memberId:m.id,member:m.name,memberPhone:m.phone||'',chitId:c.id,chit:c.name,month,amount,date,mode,note,status:paymentStatus({amount},memberMonthly(m,c)),createdAt:new Date().toISOString()};
    p.receiptNo='MQ-'+String(date).replace(/-/g,'').slice(0,6)+'-'+String(state.payments.length+1).padStart(4,'0');
...
function reports(){
  ...
  return `<div class="row"><div><h2 class="page-title">Reports</h2><div class="subtitle">Pending, daily & monthly collection</div></div><div class="report-head-actions"><button class="btn reminder-open" onclick="tab='reminders';render()">🔔 Reminders</button><input class="month-picker" type="month" value="${month}" onchange="setReportMonth(this.value)"></div></div>
